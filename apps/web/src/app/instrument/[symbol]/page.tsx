  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted mb-4">
        Price: {last?.price?.toFixed(5) ?? "—"} · {last?.dataQuality ?? "—"} · {last?.delayStatus ?? "—"}
      </p>
      ...
      <div className="grid md:grid-cols-2 gap-3 mt-6">
        <div className="border border-border rounded-md p-3 bg-panel/50">
          <h2 className="text-sm font-semibold mb-2">Retail sentiment</h2>
          {(sentiment ?? []).length === 0 ? (
            <div className="text-xs text-muted">No sentiment data.</div>
          ) : (
            sentiment!.map((s) => (
              <div key={s.source} className="text-xs mb-2">
                <div className="flex items-baseline gap-2">
                  <span className="font-semibold">{s.source}</span>
                  <span className="text-muted">delay {s.delayMinutes}m · {s.representativeness}</span>
                </div>
                ...
                <div className="flex justify-between text-[10px] text-muted mt-0.5">
                  <span>long {s.longPct.toFixed(0)}%</span>
                  <span>short {s.shortPct.toFixed(0)}%</span>
                </div>
              </div>
            ))
          )}
        ...
        <div className="border border-border rounded-md p-3 bg-panel/50">
          <h2 className="text-sm font-semibold mb-2">CFTC COT (latest)</h2>
          {(cot ?? []).length === 0 ? (
            <div className="text-xs text-muted">No COT data.</div>
          ) : (
            cot!.map((c) => (
              <div key={c.reportDate} className="text-xs grid grid-cols-2 gap-1">
                <span className="text-muted">Report date</span><span className="font-mono">{c.reportDate}</span>
                <span className="text-muted">Commercials net</span><span className="font-mono">{c.commercialsNet.toFixed(0)}</span>
                <span className="text-muted">Non-commercials net</span><span className="font-mono">{c.nonCommercialsNet.toFixed(0)}</span>
                <span className="text-muted">Weekly change</span><span className="font-mono">{c.weeklyChange.toFixed(0)}</span>
                {c.percentileRank3y != null && (
                  <>
                    <span className="text-muted">3Y percentile</span>
